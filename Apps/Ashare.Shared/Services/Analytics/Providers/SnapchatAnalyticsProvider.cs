...
public class SnapchatAnalyticsProvider : IAnalyticsProvider
{
    ...
    public SnapchatAnalyticsProvider(IOptions<AnalyticsOptions> options)
    {
        _config = options.Value.Snapchat;
    }

    public Task InitializeAsync()
    {
        if (_config.DebugMode)
            Console.WriteLine($"[Snapchat] Initialized with AppId: {_config.AppId}");
