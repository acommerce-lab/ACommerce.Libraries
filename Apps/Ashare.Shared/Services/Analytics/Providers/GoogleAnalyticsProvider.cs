...
public class GoogleAnalyticsProvider : IAnalyticsProvider
{
    ...
    public GoogleAnalyticsProvider(IOptions<AnalyticsOptions> options)
    {
        _config = options.Value.Google;
    }

    public Task InitializeAsync()
    {
        if (_config.DebugMode)
            Console.WriteLine($"[Google] Initialized with AppId: {_config.AppId}");
