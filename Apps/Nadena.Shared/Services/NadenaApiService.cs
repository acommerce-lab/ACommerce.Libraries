...
public class NadenaApiService
{
        ...
        private readonly ILogger<NadenaApiService> _logger;

    private readonly HashSet<Guid> _favorites = [];

        public NadenaApiService(
                ...
                ILogger<NadenaApiService> logger)
        {
        ...
        public async Task<BookingItem?> CreateBookingAsync(Guid spaceId, decimal totalPrice, decimal depositAmount, string rentType, string? customerId = null, Guid? hostId = null, string? paymentId = null, string? notes = null)
        {
                try
                {
                        Console.WriteLine($"[NadenaApiService] Creating booking in backend for space {spaceId}");
                        ...
                        Console.WriteLine($"[NadenaApiService] Booking request: CustomerId={customerId}, HostId={hostId}, RentType={rentTypeValue}");

                        var result = await _bookingsClient.CreateAsync(request);

                        if (result != null)
                        {
                                Console.WriteLine($"[NadenaApiService] Booking created in backend: {result.Id}");
                                return MapBookingDtoToItem(result);
                        }

                        Console.WriteLine("[NadenaApiService] Failed to create booking in backend - null response");
                        return null;
                }
                catch (Exception ex)
                {
                        Console.WriteLine($"[NadenaApiService] Error creating booking in backend: {ex.Message}");
                        return null;
                }
        }
        ...
        public void CreateBookingLocally(BookingItem booking)
        {
                ...
                Console.WriteLine($"[NadenaApiService] Local booking created: {booking.Id}");
