...
Log.Logger = new LoggerConfiguration()
    .WriteTo.Console()
    .WriteTo.File("logs/hamtramck-.txt", rollingInterval: RollingInterval.Day)
    .CreateLogger();

try
{
    Log.Information("Starting Hamtramck Hardware API...");
    ...
    {
        options.UseSqlite(connectionString ?? "Data Source=hamtramck.db");
    ...
    builder.Services.AddInMemoryMessaging("Hamtramck");
    ...
    // Seed Service
    builder.Services.AddScoped<HamtramckSeedDataService>();
    ...
    builder.Services.AddSwaggerGen(options =>
    {
        ...
        {
            Title = "Hamtramck Hardware API",
            Version = "v1.0.0",
            Description = @"
# Hamtramck Hardware - Your Local Hardware Experts Since 1952

## Features
- Browse products by category (Plumbing, Electrical, Tools, etc.)
- Search products
...
## Main Endpoints
- `/api/auth/*` - Authentication (email/phone login)
- `/api/profiles/*` - User profile management
- `/api/catalog/products/*` - Product catalog
- `/api/catalog/product-categories/*` - Product categories
...
"
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "Hamtramck Hardware API v1.0");
        options.RoutePrefix = string.Empty;
        options.DocumentTitle = "Hamtramck Hardware API";
    });
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        var seedService = scope.ServiceProvider.GetRequiredService<HamtramckSeedDataService>();
    ...
    {
        Service = "Hamtramck Hardware API",
        Status = "Healthy",
        Version = "1.0.0",
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        // Register as Marketplace for client compatibility
        await registry.RegisterAsync(new ServiceRegistration
        {
    ...
    Log.Information("Hamtramck Hardware API started successfully!");
    await app.RunAsync();
}
catch (Exception ex)
{
