...
/// </summary>
public class StorageBackedTokenStorage : ITokenStorage
{
    ...
    public StorageBackedTokenStorage(IStorageService storage)
    {
        _storage = storage;
    }
    ...
    public async Task SaveTokenAsync(string token, DateTime? expiresAt)
    {
        ...
        Console.WriteLine("[StorageBackedTokenStorage] OnTokenChanged fired immediately");
        ...
        try
        {
            ...
            Console.WriteLine("[StorageBackedTokenStorage] Token saved to persistent storage");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[StorageBackedTokenStorage] Error saving token: {ex.Message}");
        }
    }
    ...
    public async Task<(string? Token, DateTime? ExpiresAt)> GetTokenAsync()
    {
        try
        {
            ...
            {
                Console.WriteLine($"[StorageBackedTokenStorage] Token loaded from persistent storage (expires: {expiresAt})");
                // Notify subscribers that we loaded a token - updates ScopedTokenProvider's cache
                OnTokenChanged?.Invoke(token, expiresAt);
                Console.WriteLine("[StorageBackedTokenStorage] OnTokenChanged fired after loading token");
            }
        ...
        {
            Console.WriteLine($"[StorageBackedTokenStorage] Error loading token: {ex.Message}");
            return (null, null);
        }
    }
    ...
    public async Task ClearTokenAsync()
    {
        ...
        Console.WriteLine("[StorageBackedTokenStorage] OnTokenChanged (clear) fired immediately");

        try
        {
            await _storage.RemoveAsync(TokenKey);
            await _storage.RemoveAsync(ExpiresAtKey);
            Console.WriteLine("[StorageBackedTokenStorage] Token cleared from persistent storage");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[StorageBackedTokenStorage] Error clearing token: {ex.Message}");
        }
    }
