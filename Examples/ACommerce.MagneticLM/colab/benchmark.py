"""
MagneticLM Benchmark - Perplexity evaluation on standard datasets.
Supports: PTB, WikiText-2, WikiText-103, Arabic, any text file.
"""
    ...
    for idx, line in enumerate(test_lines):
        ...
        for i in range(1, len(words)):
            ...
            if mode == "bigram":
                ...
                if total > 0:
                    count = graph.ngram_counts.get(key, {}).get(words[i], 0)
                    prob = count / total
                else:
                    prob = 0
            elif mode == "kn":
...
def run_benchmark(train_path: str, test_path: str, physics_iterations: int = 50):
    """Run full benchmark on a dataset pair."""
    print("=" * 60)
    ...
    print(f"\r  Bigram:                    PPL = {ppl_bi:.1f}")

    ppl_kn = compute_perplexity(graph, test_lines, "kn")
    print(f"\r  Modified KN-5gram:         PPL = {ppl_kn:.1f}")

    ppl_cache = compute_perplexity(graph, test_lines, "cache")
    print(f"\r  KN + Cache:                PPL = {ppl_cache:.1f}")

    ppl_mag = compute_perplexity(graph, test_lines, "magnetic")
    print(f"\r  MagneticLM (full):         PPL = {ppl_mag:.1f}")
    ...
    print(f"  {'Our KN + Cache':<35}| {ppl_cache:.1f}")
    ...
    best = min(ppl_kn, ppl_cache, ppl_mag)
    if best < 52:
        print("\n  >>> BEAT AWD-LSTM + Cache! <<<")
    elif best < 78:
        print("\n  >>> BEAT LSTM! <<<")
    ...
    return graph, ppl_bi, ppl_kn, ppl_cache, ppl_mag
