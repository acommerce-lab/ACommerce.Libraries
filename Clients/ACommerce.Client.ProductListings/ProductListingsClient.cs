...
public sealed class ProductListingsClient(DynamicHttpClient httpClient)
{
	...
	{
		return await httpClient.GetAsync<List<ProductListing>>(ServiceName, "/api/listings", cancellationToken);
	}

	public async Task<ProductListing?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default)
	{
		return await httpClient.GetAsync<ProductListing>(ServiceName, $"/api/listings/{id}", cancellationToken);
	}

	public async Task<List<ProductListing>?> GetByVendorAsync(Guid vendorId, CancellationToken cancellationToken = default)
	{
		return await httpClient.GetAsync<List<ProductListing>>(ServiceName, $"/api/listings/vendor/{vendorId}", cancellationToken);
	}

	public async Task<ProductListing?> CreateAsync(CreateListingRequest request, CancellationToken cancellationToken = default)
	{
		return await httpClient.PostAsync<CreateListingRequest, ProductListing>(ServiceName, "/api/listings", request, cancellationToken);
	}
	...
	{
		return await httpClient.PutAsync<UpdateListingRequest, ProductListing>(ServiceName, $"/api/listings/{id}", request, cancellationToken);
	}
