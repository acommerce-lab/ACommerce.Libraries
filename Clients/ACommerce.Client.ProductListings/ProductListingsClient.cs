...
public sealed class ProductListingsClient(IApiClient httpClient)
{
    ...
    public async Task<List<ProductListingDto>?> GetAllAsync(CancellationToken cancellationToken = default)
        ...
        {
                var response = await httpClient.GetAsync<SearchListingsResponse>(ServiceName, $"/api/listings/by-vendor/{vendorId}", cancellationToken);
                return response?.Items;
        }

        public async Task<List<ProductListingDto>?> GetMyListingsAsync(CancellationToken cancellationToken = default)
        {
                return await httpClient.GetAsync<List<ProductListingDto>>(ServiceName, "/api/listings/my-listings", cancellationToken);
        }
