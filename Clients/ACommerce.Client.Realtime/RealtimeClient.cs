...
public sealed class RealtimeClient : IAsyncDisposable
{
    ...
    public async Task EnsureConnectedAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_lastServiceName != null && _lastHubPath != null)
        {
            _logger.LogInformation("Reconnecting to SignalR hub...");
            _isConnected = false;
            await ConnectAsync(_lastServiceName, _lastHubPath, _lastAccessToken, cancellationToken);
        }
        else
        {
            throw new InvalidOperationException("No previous connection to restore. Call ConnectAsync first.");
        }
    }
